...
update_details() {
		...
		grep -Eiw "$STUDENTID" students-list_0923.txt
		echo "Enter your previous Age"
                read -r OLDAGE
                echo "Enter your new Age"
                read -r NEWAGE
                awk -v ID="$STUDENTID" -v oldage="$OLDAGE" -v newage="$NEWAGE" '/ID/ {sub(oldage, newage, $0)} 1' students-list_0923.txt > tempfile && mv tempfile students-list_0923.txt
		...
		grep -Eiw "$STUDENTID" students-list_0923.txt
                ...
                read -r NEWEMAIL
                awk -v ID="$STUDENTID" -v prevemail="$PREVEMAIL" -v newemail="$NEWEMAIL" '/ID/ {sub(prevemail, newemail, $0)} 1' students-list_0923.txt > tempfile && mv tempfile students-list_0923.txt
